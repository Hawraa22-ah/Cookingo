...
-- Safely drop any existing conflicting policies
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Anyone can view recipes" ON recipes;
    DROP POLICY IF EXISTS "Chefs can insert their own recipes" ON recipes;
...
BEGIN
    ...
    -- Only chefs can insert their own recipes
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'recipes' 
          AND policyname = 'Chefs can insert their own recipes'
    ...
    -- Chefs can update their own recipes
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'recipes' 
          AND policyname = 'Chefs can update their own recipes'
    ...
    -- Chefs can delete their own recipes
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'recipes' 
          AND policyname = 'Chefs can delete their own recipes'
...
-- Seed example recipe if a chef exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM profiles WHERE role = 'chef') AND NOT EXISTS (
        SELECT 1 FROM recipes WHERE id = '223e4567-e89b-12d3-a456-426614174001'
    ) THEN
        INSERT INTO recipes (
          id,
          title,
