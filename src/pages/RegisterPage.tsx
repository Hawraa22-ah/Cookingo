...
const RegisterPage: React.FC = () => {
  ...
  const [role, setRole] = useState<'user' | 'chef' | 'seller' | 'admin'>('user');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!['user', 'chef', 'seller', 'admin'].includes(role)) {
        ...
        <div className="max-w-md w-full space-y-6">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {[
              ...
              {
                roleType: 'admin',
                icon: <Lock className="w-6 h-6 mb-1" />,
                label: 'Admin',
                description: 'Full access including user features and dashboard',
              },
            ].map(({ roleType, icon, label, description }) => (
              <button
                key={roleType}
                type="button"
                onClick={() => setRole(roleType as 'user' | 'chef' | 'seller' | 'admin')}
